...
<head>
    ...
    <title>Gráfico de Saídas por Responsável</title>
    ...
    $sqlDia = "SELECT responsavel, valor FROM saidas";
    $result = mysqli_query($conn, $sqlDia);

    $data = [];
    $data[] = ['Responsável', 'Valor'];
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (isset($somaResponsaveis[$responsavel])) {
            $somaResponsaveis[$responsavel] += $valor;
        } else {
            $somaResponsaveis[$responsavel] = $valor;
        }
    }

    foreach ($somaResponsaveis as $responsavel => $valor) {
        $data[] = [$responsavel, $valor];
    }
