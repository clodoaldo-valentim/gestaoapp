...
$mes = $_POST['mes']; // Outubro
$ano = $_POST['ano'];
$sqlMes = "SELECT * FROM entradas WHERE MONTH(data) = ? AND YEAR(data) = ?";
$stmtMes = $conn->prepare($sqlMes);
$stmtMes->bind_param("ii", $mes, $ano);
$stmtMes->execute();
$resultadoMes = $stmtMes->get_result();

echo "<h1 class='text-center'>Resultado da consulta $mes/$ano</h1>";
...
echo "</thead>";
echo "<tbody>";
while($row = $resultadoMes->fetch_assoc()) {
  echo "<tr>";
  echo "<td>".$row["id"]."</td>";
