...
<head>
    ...
    <title>Gráfico de Saídas por Ano</title>
    ...
    $sqlDia = "SELECT data, valor FROM entradas";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $ano = date('Y', strtotime($row['data']));
        $valor = (float)$row['valor'];

        if (isset($somaAnos[$ano])) {
            $somaAnos[$ano] += $valor;
        } else {
            $somaAnos[$ano] = $valor;
        }
    ...
    foreach ($somaAnos as $ano => $valor) {
        $data[] = [(string)$ano, $valor];
    }
